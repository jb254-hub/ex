...
if not MASTER_ADMIN_KEY:
    raise ValueError("MASTER_ADMIN_KEY is required in .env")
...
DATABASE_NAME = os.getenv("DATABASE_NAME", "exegesis")

if not MONGODB_URI:
    raise ValueError("MONGODB_URI is required in .env")
...
async def validate_api_key(
    ...
    return str(record["developer_id"])
...
async def lifespan(
    ...
    print(
        "🚀 Exegesis API started (MongoDB ready)"
    )
...
app = FastAPI(
    title="Exegesis API",
    version="1.0.0",
    description="Exegesis AI API powered by Groq",
    lifespan=lifespan
)
...
async def generate_key(
    ...
    return {
        ...
        "message":
            "API key generated successfully. Store it securely."
    }
